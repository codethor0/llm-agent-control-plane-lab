...
| Docker validation | Environment drift; non-reproducible validation | `Dockerfile`, `docker-compose.yml`; CI docker job | CI: `docker compose build` + `pytest` (299 tests); local `make validate` | Implemented, tested in CI | Docker optional locally; image does not bind-mount host code |
...
| Release-readiness doc alignment | Stale test counts; false release claims in ops docs | `reports/final-release-readiness-audit.md`; operational checklists | `tests/test_release_readiness_docs.py` | Documented (audit) | Audit report is point-in-time; re-run before each release |
