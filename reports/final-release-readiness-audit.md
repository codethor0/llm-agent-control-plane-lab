...
| Item | Status |
|------|--------|
| README latest release | Promoted to **v0.2.9** in Iteration 2 |
| Social launch copy | Removed from README; `docs/social/` deleted |
| Core idea | "LLM agents may propose actions, but only the external control plane can authorize them." |
| Logo | `docs/assets/llm-agent-control-plane-logo.svg` added |
...
| Branch protection | Documented; **not enabled** (API 404) |
...
| Finding | Action |
|---------|--------|
| Stale **271** / **210** in release checklists and some diagrams | Updated to **293** |
| Social launch copy in repo | Removed (README + `docs/social/`) |
| `main` branch protection | Not enabled; documented in `docs/branch-protection.md` |
...
1. Tag **v0.2.9** after `main` CI is green on the release commit.
2. Verify `release-artifacts` workflow and `sha256sum -c SHA256SUMS` for v0.2.9.
3. Enable `main` branch protection per `docs/branch-protection.md`.
4. Triage Dependabot PRs (#4-#8) post-release.
